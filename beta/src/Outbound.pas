...
begin
  ...
  if Length(S) < 2 then
  begin
    Result := kaBsoNothingAfter;
    S := '';
    Exit
  end;
  Result := TKillAction(Pos(S[1], SKillActionB));
  if Result <> kaBsoNothingAfter then
    DelFC(S);
  case S[1] of
    ';', '~': begin
      S := '';
...
begin
  ...
  begin
    ...
    if uFindFirstEx(fn, SR, FindExSearchLimitToDirectories) then
    begin
      repeat
    ...
    case os of
      ...
      osError : ;
        ...
        begin
          ...
          f.Address := Address;
          f.FStatus := os;
          if Files = nil then Files := TOutFileColl.Create('Files');
          Files.Add(f);
        end;
     end;
  end;
