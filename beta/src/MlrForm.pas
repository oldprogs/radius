...
uses
   ...
   Dialogs, ImgList, RemoteUnit, xOutline, Grids;
...
//    procedure tpCreateNormalPollClick(Sender: TObject);
    ...
    //    procedure mcShowIconClick(Sender: TObject);
...
{$ENDIF}
    ...
    procedure WMTHEMECHANGED(var M: TMessage); message WM_THEMECHANGED;
    procedure DrawThemedIcons;
...
type
   ...
   T_OpenUniTerm = function(const PortHandle: THandle): Boolean; stdcall;
...
uses
   ...
   DupCfg, FidoPwd, NodeBrws, StupCfg, NodeLCfg, wizard, {fileutil,} plus, util,
   ...
   begin
      ...
      begin
         ...
         begin
            n.Checked := True;
            tn.Checked := True;
            //        n.Tag:= MT.IdxOf(l.Name);
            //        tn.Tag:= MT.IdxOf(l.Name);
            //        n.Enabled := False;
         end
         else
         begin
...
begin
   try
      ...
      if (MailerThreads <> nil) then begin
         if (MailerThreads.Count = 0) then ShowIt('Idle', False)
...
{  msgc.Msg := Msg.message;
  msgc.WParam := Msg.wParam;
  msgc.lParam := Msg.lParam;
  msgs := TWMSize(msgc);
  if (msgs.Msg = WM_SIZE) and (msgs.SizeType = SIZE_MINIMIZED) then
  begin
...
begin
  ...
  with TMailerForm(Application.MainForm).TermTx do
  begin
    cw := ClientWidth;
    ch := ClientHeight
  end;
               ...
               case msg.WParam of
                  RASCS_CONNECTED:
                     begin
                        ...
                        _RecalcPolls(True);
                        if IniFile.rasConnFlag then
                          CreateFlag('rasdial.connected');
                        PlaySnd('RASConnect', tlNotDefined);
                     end;
                  RASCS_DISCONNECTED:
                     begin
                        PlaySnd('RASFinish', tlNotDefined);
                        if RASConnect then
                        begin
                           RASConnect := False;
                        ...
                        if IniFile.rasDisconnFlag then
                          CreateFlag('rasdial.disconnected');
                     end;
               end;
...
begin
  Result := ((Message.Msg = WM_ENDSESSION) and (Message.LParam = integer(ENDSESSION_LOGOFF)));
{  if (Message.Msg = WM_SIZE) and (TWMSize(Message).SizeType = SIZE_MINIMIZED) then
  begin
    Application.OnMinimize(Self);
  end;}
  if (Message.Msg = WM_SENTWINM) then
    Application.OnMinimize(Self);
end;
...
begin
  FillBWListBox;
  j := 0;
  for i := wcb_TopPage0 to wcb_TopPage5 do
  begin
...
begin
  j := 0;
  for i := wcb_BtmPage0 to wcb_BtmPage5 do
  begin
...
begin
   ...
   llSessionCost.Left := lSessionCost.Left - llSessionCost.Width - 6;
...
{$ENDIF}
   ...
   if ActiveLine = PanelOwnerInfo then ti := c;
   DoAdd(LngStr(rsMMInfoPanel));
{$IFDEF WS}
   if DaemonStarted then
   begin
...
begin
  ...
  begin
    ...
    if L is TMenuItem then Mnu.Caption := s
    else if L is TEdit then Edt.Text := s
    else GlobalFail('TMailerForm.SetLabel(%s,...,"%s")', [L.ClassName, s]);
end;
...
{$IFDEF WS} or (ActiveLine = PanelOwnerDaemon){$ENDIF}
   ...
   //  Seconds:=(uGetSystemTime - ActiveLine.SD.ConnectStart);
   //  SetLabel(lSessionTime, wcslSessionTime, FormatDateTime('hh:mm:ss',Seconds/86400));
   ...
   begin
    ...
    begin
      ...
      begin
        if ela > 0 then i := DWORD(a) div ela
        else i := 0;
            //      i := T.SmartCPS; // visual
...
begin
   if (ActiveLine = PanelOwnerPolls) or
      ...
      (ActiveLine = PanelOwnerInfo) 
{$IFDEF WS} or (ActiveLine = PanelOwnerDaemon){$ENDIF}
   or (MailerThreads.IndexOf(ActiveLine) = -1)
   or (ActiveLine.CP = nil) then begin
...
procedure TMailerForm.UpdateView(fromcc: boolean);
   ...
   begin
      ...
      for k := 0 to evListView.Items.Count - 1 do
      begin
      ...
      FreeObject(c);
      for k := evListView.Items.Count - 1 downto 0 do
      begin
        if evListView.Items[k].DropTarget then
        begin
      ...
      EdtEvtBtn.Enabled := evListView.ItemIndex > -1;
   end;
...
{$ENDIF}
   ...
   const
      PT: array[TPollType] of Integer = (-1, rsMMptAuto, rsMMptCron, rsMMptManual, rsMMptImm, rsMMptBack{$IFDEF EXTREME}, rsMMptRCC{$ENDIF});
   var
      C: TColl;
      S: TStringColl;
      I, N: Integer;
...
{$ENDIF}
   ...
   var
      ...
      t: string;
   begin
      B := bwListView.Selected <> nil;
      SetEnabledO(bKillBWZ, wcb_bKillBWZ, B);
      t := '';
      if stListView.ItemFocused <> nil then
      begin
        t := stListView.ItemFocused.Caption;
      end;
      for i := 0 to stListView.Items.Count - 1 do
      begin
      ...
      begin
         ...
         a.DropTarget := False;
         if a.SubItems[0] <> m.StatusMsg then a.SubItems[0] := m.StatusMsg;
...
var
  ClearMlr, B, OutMgrTab{, InfoTab}: Boolean;
  dsk: byte;
begin
...
{$ENDIF}
   ...
   {r48}
   //Свободное место в инбоксе by 2:5020/1970
...
{$ENDIF}
    ...
    Integer(PanelOwnerInfo):   UpdateSystem;
    Integer(PanelOwnerPolls):  if not fromcc then UpdatePolls;
  else
    begin
      if MailerThreads.IndexOf(ActiveLine) = -1 then
      begin
        exit;
...
begin
   ...
   mbp.lpszText := PChar(
      ProductNameFull + ', ' + LngStr({$IFDEF WS}rsAboutTCP{$ELSE}rsAboutDialup{$ENDIF}) + #13#10 +
      FormatLng(rsAboutVersion, [ProductVersion]) + ', ' + ProductDate + #13#10#13#10 +
      'Copyright c 2000-2005 by Denis Voituk. All rights reserved'#13#10 +
      'FidoNet: 2:5012/38, 2:5012/37, 2:550/5012'#13#10 +
      'E-Mail: deniska666@mail.ru'#13#10 +
      'Copyright c 2003-2005 by Alex Shakhaylo. All rights reserved'#13#10 +
      'FidoNet: 2:461/700, 2:461/701, 2:461/702'#13#10 +
      'Copyright © 2001 OlegON software. All rights reserved'#13#10 +
      'FidoNet: 2:5020/1970'#13#10 +
      'E-Mail: support@olegon.com'#13#10#13#10 +
      'Partial Copyright © 2001-2002 by Sergei Shumakov. All rights reserved'#13#10 +
      'FidoNet: 2:5005/47'#13#10#13#10 +
      'Copyright © 1996-2001 RITLABS S.R.L. All rights reserved'#13#10#13#10 +
      'Contains cryptographic software written by Eric Young');

   mbp.lpszCaption := 'About Mailer';
   mbp.hInstance := hInstance;
   mbp.lpszIcon := MAKEINTRESOURCE(5012);
   mbp.dwStyle := MB_USERICON or MB_OK;
   mbp.dwContextHelpId := 0;
   //  mbp.lpfnMsgBoxCallback;
...
begin
  lInfo1.Caption := IniFile.LastIn + #32#13#10 + IniFile.LastOut + #32;
  ...
  bmpname := MakeNormName(IniFile.HomeDir, 'MainMenu.bmp');
  bmp := TBitmap.Create;

   try
     if IniFile.ShowMenuIcons then
     begin
       if FileExists(bmpname) then
       begin
         bmp.LoadFromFile(bmpname);
         ilMainMenu.Clear;
         ilMainMenu.AddMasked(bmp, clFuchsia);
       end else //no skin file
       begin
         bmp.LoadFromResourceName(HInstance, 'MAINMENU_BMP');
         ilMainMenu.Clear;
         ilMainMenu.AddMasked(bmp, clFuchsia);
       end
     end else //do not show icons
     begin
       ilMainMenu.Clear;
     end;
   finally
   ...
   TopPagePanels[wcb_TopPage5] := InfoPanel;
   ...
   BtmPagePanels[wcb_BtmPage5] := InfoBtnPanel;
...
{$ENDIF}
  ...
  begin
//     wcb := [wcb_lSndSize .. wcb_mlSendMdmCmds];
//     Include(wcb, wcb_TopPage0);
//     Include(wcb, wcb_TopPage1);
//     Include(wcb, wcb_TopPage2);
//     Include(wcb, wcb_TopPage3);
//     Include(wcb, wcb_TopPage4);
...
//    LoadOutMgrBmp(ioLines, 'LINES', clWhite);
    ...
    {    fmFlags.FolderName := IniFile.FlagsDir;
        fmFlags.Active := true;}
  end;

  Include(wcb, wcb_TopPage5);
  DrawThemedIcons;
  ...
  InitOutMgrPopup;
   ...
   begin
      ...
      LogBox.Font.Height := LoggerFontHeight;
...
begin
   if (ActiveLine.CP <> nil) and ((ActiveLine is TMailerThread){$IFDEF WS} and (ActiveLine.DialupLine){$ENDIF}) then
   begin
     if @F_OpenUniTerm <> nil then
     begin
...
begin
   case M.LParam of
      0: BWListView.GridLines := IniFile.GridInBWZ;
      1: PollsListView.GridLines := IniFile.GridInPV;
   else {error}
      ;
   end; {of case}
end;

procedure TMailerForm.DrawThemedIcons;
var
  R2: TRect;
...
begin
  LoadOutMgrBmp(ioLines, 'LINES', clWhite);
  if InitThemeLibrary and IsThemeActive then
  begin
    for i := 3 to 5 do
    begin
      Details := ThemeServices.GetElementDetails(ttGlyphClosed);
      R2 := Rect(i * 16, 0, (i + 1) * 16, 16);
      ThemeServices.DrawElement(OutMgrBmps[ioLines].Canvas.Handle, Details, R2, nil);
    end;
...
procedure TMailerForm.WMTHEMECHANGED(var M: TMessage);
begin
  bDrawThemedIcons := true;
  OutMgrOutline.Repaint;
end;
...
const
   PtpTyp: array[TPollType] of Integer = (0, rsMMptpiOutb, rsMMptpiCron, rsMMptpiManual, rsMMptpiImm, rsMMptpiBack{$IFDEF EXTREME}, rsMMptpiRCC{$ENDIF});
...
begin
   ...
   //  addsize:=false;
   if bDrawThemedIcons then
   begin
     DrawThemedIcons;
     bDrawThemedIcons := false;
   end;
   ...
   begin
      ...
      begin
        if (f.Nfo.Attr and olfLastItem = 0) then I := 0
        else I := 1;
        if (f.Nfo.Attr and olfLastLevel = 0) then BitBlt(Handle, R.Left, R.Top, 16, 16, OutMgrBmps[ioLines].Canvas.Handle, 144, 0, SRCCOPY)
          ...
          Brush.Color := clWindow;
          Font.Color := clWindowText;
          FillRect(Rect(R.Left, R.Top, 16, 16));
          Brush.Color := col1;
          Font.Color := col2;
        end;
        Inc(R.Left, 16);
      ...
      R1 := R;
{      R1.Right := OutMgrHeader.Sections[0].Width +
                  OutMgrHeader.Sections[1].Width +
                  OutMgrHeader.Sections[2].Width +
                  OutMgrHeader.Sections[3].Width +
                  OutMgrHeader.Sections[4].Width +
                  OutMgrHeader.Sections[5].Width;}
      FillRect(R1);
      Inc(R.Left, 4);
   ...
   //  Outbsize:=outbsize+F.Nfo.Size;
   //  lOutBoundSize.Caption:=Format('%d Mb',[Outbsize]);
   //  if addsize then lOutBoundSize.Caption:=FloatToStr(Round(Outbsize/1048576))+' Mb';
end;
...
begin
   ...
   InfoPanel.Width := MainPanel.ClientWidth;
   InfoPanel.Height := BottomPanel.Top - InfoPanel.Top;
...
begin
   Application.HelpFile := GetHelpFile(HomeDir, LangNames[lnDutch]);
   HelpLanguageId := HelpLanguageDutch;
   SetRegHelpLng(LangNames[lnDutch]);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if (Application.MainForm = Self) and (M.Msg = WM_SYSCOMMAND) and (M.WParam = SC_MINIMIZE) then
      begin
        TrayIcon.Active := not IniFile.Stealth;
...
{$ENDIF}
         ...
         Integer(PanelOwnerInfo): Data := IDH_INFO;
       else
         Data := IDH_MLRD;
       end; {of case}
     end;
...
begin
   ...
   //  TrayPopupMenu.Assign(MainMenu);
   //  TrayPopupMenu.Items.
   //  Assign(MainMenu.Items);
   //  TPopupMenu(MainMenu).Popup(0,0);
   //  TrayPopupMenu.Popup(0,0);
   //  TrackPopupMenu(MainMenu.Handle,TPM_LEFTALIGN,0,0,0,Self.Handle,nil);
end;
...
begin
   if MailerForms = nil then Exit;
   for i := 0 to MailerForms.Count - 1 do
      with TMailerForm(MailerForms.At(i)) do
      begin
         SndTot.BackColor := IniFile.GaugeBack;
         RcvTot.BackColor := IniFile.GaugeBack;
         SndTot.ForeColor := IniFile.GaugeFore;
         ...
         LogBox.Font.Height := IniFile.LoggerFontHeight;
...
procedure TMailerForm.mnuTaurusOnWebClick(Sender: TObject);
begin
   ShellExecute(Handle, { handle to parent window }
      nil, { pointer to string that specifies operation to perform }
...
procedure TMailerForm.stListViewDblClick(Sender: TObject);
begin
  if stListView.ItemIndex < 0 then exit;
  ActiveLine := TMailerThread(MailerThreads[stListView.ItemIndex]);
  PostMsg(WM_UPDATETABS);
  PostMsg(WM_TABCHANGE);
end;
