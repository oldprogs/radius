...
uses
  Controls, Windows, SysUtils, StdCtrls, Classes,
  Graphics, ComCtrls, Messages, Extctrls, Menus;
...
type
  TProtCore = (ptUndefined, ptDialup, ptifcico, ptTelnet, ptBinkP{$IFDEF EXTREME}, ptFTP, ptHTTP, ptSMTP, ptPOP3, ptGATE{$ENDIF});
  TSessionCore = (scUndefined, scFTS1, scEmsiWz, scBinkP{$IFDEF EXTREME}, scFTP, scHTTP, scSMTP, scPOP3, scGATE{$ENDIF});

  TLangName = (lnEnglish, lnRussian, lnSpanish, lnDutch, lnGerman, lnDanish);

  TTernarLogic = (tlFalse, tlNotDefined, tlTrue);
...
const
  ...
  CProductVersionA  = '4.011';
  CProductMinorVersion = 11;
  CProductMajorVersion = 4;
  CProductName     = 'Radius';
//  CReleaseSpec     = {$IFDEF EXTREME} 'Extreme-Alfa' {$ELSE} 'TAPIBeta' {$ENDIF};
  CReleaseSpec     = 'Alpha';

//  CProductNameVer  = CProductName + ' v' + CProductVersion;
  CProductNameFull = 'Radius by Denis Voituk (based on Argus)';
  ...
  LangNames : array [TLangName] of ShortString = ('en', 'ru', 'es', 'nl', 'de', 'da');
...
//    FreeOnTerminate: Boolean;
  ...
  public
    ...
    function WaitEvtAEx(nCount: Integer; lpHandles: PWOHandleArray; Timeout: DWORD; AWaitAll: Boolean; eName: string = ''): DWORD;
    function WaitEvtA(nCount: Integer; lpHandles: PWOHandleArray; Timeout: DWORD; eName: string = ''): DWORD;
    function WaitEvts(const id: array of THandle; Timeout: DWORD; eName: string = ''): DWORD;
    function WaitEvtsAll(const id: array of THandle; Timeout: DWORD; eName: string = ''): DWORD;
...
{$IFDEF DEBUG_VERSION}
  ...
  public
    constructor Create (var extCS: TRTLCriticalSection);
    destructor Destroy; override;
...
{$IFDEF DEBUG_VERSION}
  ...
  EnterListCS: TRTLCriticalSection;
...
uses Forms, ShellAPI, RRegExp, igHHInt, NTdyn, Util, xMisc, RadIni, wizard
{$IFDEF DEBUG_VERSION}
  ,JCLDebug
{$ENDIF}
;
...
begin
  ...
  if (LiveCounter > 20 * 60 * 2) or (WindCounter > 20 * 60 * 2) and IniFile.UseAntiHang then begin
     GlobalFail('Radius hanged up: %d %d', [LiveCounter, WindCounter]);
  end;
...
begin
   ...
   if GetVersionEx(OS) then begin
      ...
      if OS.szCSDVersion[0] <> #0 then begin
...
//        Str(L / 1000:0:1, Result);
        Str(L / 1024:0:1, Result);
        Result := Result + 'K';
      end;
    1000000..999999999:
      begin
//        Str(L / 1000000:0:1, Result);
        Str(L / 1048576:0:1, Result);
        Result := Result + 'M';
      end;
    else
      begin
//        Str(L / 1000000:0:1, Result);
        Str(L / 1073741824:0:1, Result);
        Result := Result + 'G';
      end;
  end;
end;
...
function T_Thread.WaitEvtAEx(nCount: Integer; lpHandles: PWOHandleArray; Timeout: DWORD; AWaitAll: Boolean; eName: string = ''): DWORD;
{$IFDEF DEBUG_VERSION}
var
   st: string;
   en: string;
...
{$IFDEF DEBUG_VERSION}
   if (IniFile <> nil) and IniFile.logWaitEvt then begin
      if TimeOut = INFINITE then St := 'INFINITE' else St := IntToStr(TimeOut);
      if eName <> '' then St := St + ', ' + eName;
...
function T_Thread.WaitEvtA(nCount: Integer; lpHandles: PWOHandleArray; Timeout: DWORD; eName: string = ''): DWORD;
begin
  Result := WaitEvtAEx(nCount, lpHandles, Timeout, False);
end;
...
function T_Thread.WaitEvtsAll;
begin
  Result := WaitEvtAEx(High(id) + 1, @id, Timeout, True);
end;

function T_Thread.WaitEvt(H: THandle; Timeout: DWORD; eName: string = ''): DWORD;
begin
   Result := WaitEvts([H], Timeout);
end;
...
begin
  ...
  {до просветления, почему postmessage может возвращать 0}
end;
...
// Prepare Disp & Flags
  ...
  // Prepare 'Access' and 'Share'
    ...
    if cShare in Mode then Share := FILE_SHARE_READ or FILE_SHARE_WRITE;
...
begin
   ...
   for n := i downto 0 do begin
      Result := Result + ModuleByLevel(n + 1) + '.' + ProcByLevel(n + 1) + '(' + IntToStr(LineByLevel(n + 1)) + '), ';
   end;
end;
{$ENDIF}
...
{$IFDEF DEBUG_VERSION}
  ...
  begin
    ...
    begin
      if pos(Pad('Enter_(pre)', 19) + ': EnterCS, ' + IntToStr(Integer(CS.DebugInfo)), EnterList[i]) > 0 then
      begin
...
{$IFDEF DEBUG_VERSION}
   if (IniFile <> nil) and IniFile.logWaitEvt then begin
      ...
      for i := 0 to EnterList.Count - 1 do begin
         if pos(Pad('Enter', 19) + ': EnterCS, ' + IntToStr(Integer(CS.DebugInfo)), EnterList[i]) > 0 then begin
            EnterList.Delete(i);
            break;
         end;
      end;
...
begin
{$IFDEF DEBUG_VERSION}
  if Debug = nil then Debug := TEnterList.Create (DebugCS);
{$ENDIF}  
...
begin
  ...
  fName := ClassName;
{$IFDEF DEBUG_VERSION}
  Debug := TEnterList.Create (DebugCS);
{$ENDIF}
  DoInit(32, 64);
end;
...
begin
   ...
   for i := 0 to Count - 1 do begin
      if pos(UpperCase(Str), UpperCase(Self[i])) = 1 then begin
         Result := i;
         exit;
      end;
   end;
...
begin
   While fObjects.Count < Count do fObjects.Add(nil);
end;
...
begin
  Result := MakeNormName(HomePath, Format('radius%s.%s', [Lng, ext[IsHtmlHelp and not HtmlHelpLibError]]));
end;
...
{$IFDEF DEBUG_VERSION}
constructor TEnterList.Create (var extCS: TRTLCriticalSection);
begin
   inherited Create;
   CS := @extCS;
...
begin
   ...
   m := 0;
   if ssAlt in s then m := m or MOD_ALT;
   if ssCtrl in s then m := m or MOD_CONTROL;
   if ssShift in s then m := m or MOD_SHIFT;
...
{$IFDEF DEBUG_VERSION}
  EnterList := TEnterList.Create (EnterListCS);
{$ENDIF}
  LiveCounter := 0;
  WindCounter := 0;
  ScanCounter := 0;
