...
{ The TOutlineNode is an encapsulation of an outliner item.  Access
  ...
  protected
    constructor Create(AOwner: TxCustomOutline);
    ...
    property Outline: TxCustomOutline read FOutline;
...
{ The TxCustomOutline object is a container class for TOutlineNodes.
  ...
  the TxCustomOutline using the following properties:
    CurItem
      Reads and writes the current item
    ItemCount
      Returns the total number of TOutlineNodes with the TxCustomOutline.
  ...
  The TxCustomOutline has a number of properties which will affect all
  TOutlineNodes owned by the TxCustomOutline:
    OutlineStyle
  ...
  TxCustomOutline = class(TAdvCustomGrid)
  private
...
//    FMaskColor: TColor;
    ...
    function AttachNode(Index: LongInt; const Str: string;
    ...
    procedure DrawPictures(const BitMaps: array of TBitmap; ARect: TRect);
...
constructor TOutlineNode.Create(AOwner: TxCustomOutline);
begin
  FOutline := AOwner;
end;
...
constructor TxCustomOutline.Create(AOwner: TComponent);
begin
...
procedure TxCustomOutline.Init;
begin
...
procedure TxCustomOutline.CreateGlyph;
var
  I: Integer;
begin
  FUserBitmaps := [];
...
procedure TxCustomOutline.SetRowHeight;
var
  ScreenDC: HDC;
begin
...
procedure TxCustomOutline.Clear;
begin
  FRootNode.Destroy;
  FRootNode := nil;
  Init;
...
procedure TxCustomOutline.DefineProperties(Filer: TFiler);
  ...
  var
    Ancestor: TxCustomOutline;
  begin
    Ancestor := TxCustomOutline(Filer.Ancestor);
...
procedure TxCustomOutline.ReadBinaryData(Stream: TStream);
begin
  Stream.ReadBuffer(FOldBitmaps, SizeOf(FOldBitmaps));
end;

procedure TxCustomOutline.WriteBinaryData(Stream: TStream);
begin
  Stream.WriteBuffer(FuserBitmaps, SizeOf(FUserBitmaps));
end;

function TxCustomOutline.IsCurItem(Value: LongInt): Boolean;
begin
  Result := Value = FCurItem.Index;
end;

function TxCustomOutline.GetItemCount: LongInt;
begin
  Result := FRootNode.GetLastIndex;
end;

procedure TxCustomOutline.MoveNode(Destination, Source: LongInt;
  AttachMode: TAttachMode);
var
...
function TxCustomOutline.AttachNode(Index: LongInt; const Str: string;
  Ptr: Pointer; AttachMode: TAttachMode): LongInt;
var
...
function TxCustomOutline.Get(Index: LongInt): TOutlineNode;
begin
  Result := FCurItem;
  if not IsCurItem(Index) then
    try
...
function TxCustomOutline.GetSelectedItem: LongInt;
begin
...
procedure TxCustomOutline.ResetSelectedItem;
begin
  FSelectedItem := FRootNode;
end;

procedure TxCustomOutline.SetRowFromNode(Node: TOutlineNode);
var
  RowValue: LongInt;
begin
...
procedure TxCustomOutline.SetSelectedItem(Value: Longint);
var
  Node: TOutlineNode;
begin
...
function TxCustomOutline.Insert(Index: LongInt; const Text: string): LongInt;
begin
  Result := InsertObject(Index, Text, nil);
end;

function TxCustomOutline.InsertObject(Index: LongInt; const Text: string; const Data: Pointer): LongInt;
begin
  Result := -1;
...
function TxCustomOutline.Add(Index: LongInt; const Text: string): LongInt;
begin
  Result := AddObject(Index, Text, nil);
end;

function TxCustomOutline.AddObject(Index: LongInt; const Text: string; const Data: Pointer): LongInt;
begin
  Result := -1;
...
function TxCustomOutline.AddChild(Index: LongInt; const Text: string): LongInt;
begin
  Result := AddChildObject(Index, Text, nil);
end;

function TxCustomOutline.AddChildObject(Index: LongInt; const Text: string; const Data: Pointer): LongInt;
begin
  Result := -1;
...
procedure TxCustomOutline.Delete(Index: LongInt);
begin
  if Index > 0 then
  begin
    try
...
procedure TxCustomOutline.Move(Destination, Source: LongInt; AttachMode: TAttachMode);
begin
...
procedure TxCustomOutline.DeleteNode(Node: TOutlineNode; CurIndex: LongInt);
begin
  if (FGoodNode = FRootNode) and (FRootNode.List.Count <> 0) then
...
procedure TxCustomOutline.SetLevel(Node: TOutlineNode; CurLevel, NewLevel: Cardinal);
var
  NumLevels: Integer;
...
procedure TxCustomOutline.Click;
begin
...
procedure TxCustomOutline.WMSize(var Message: TWMSize);
begin
  inherited;
  ...
  case Key of
    ...
    VK_RIGHT:
      begin
        ...
        begin
          if (Node.HasItems) and (Node.Expanded) then SelectedItem := SelectedItem + 1
    ...
    VK_LEFT:
      begin
        ...
        begin
          if (not Node.HasChildren) and (not (Node.Parent = FRootNode)) then SelectedItem := Node.Parent.Index
...
procedure TxCustomOutline.DblClick;
var
  Node: TOutlineNode;
begin
  inherited DblClick;
...
procedure TxCustomOutline.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TxCustomOutline.FullExpand;
begin
  FRootNode.FullExpand;
end;

procedure TxCustomOutline.FullCollapse;
var
  I: Integer;
begin
...
procedure TxCustomOutline.SetHorzScrollBar;
begin
...
procedure TxCustomOutline.DoExpand(Node: TOutlineNode);
begin
  with Node do
    Expanded := not Expanded;
end;

procedure TxCustomOutline.BeginUpdate;
begin
...
procedure TxCustomOutline.EndUpdate;
begin
  Dec(UpdateCount);
...
procedure TxCustomOutline.SetUpdateState(Value: Boolean);
begin
...
function TxCustomOutline.ResizeGrid: Boolean;
var
  OldRowCount: LongInt;
begin
  Result := False;
...
function TxCustomOutline.BadIndex(Value: TOutlineNode): Boolean;
begin
  Result := CompareNodes(Value, FGoodNode) = ocGreater;
end;

function TxCustomOutline.SetGoodIndex(Value: TOutlineNode): TOutlineNode;
var
  ParentNode: TOutlineNode;
  Index: Integer;
...
function TxCustomOutline.CompareNodes(Value1, Value2: TOutlineNode): TOutlineNodeCompare;
var
  Level1: Integer;
  Level2: Integer;
  Index1: Integer;
...
function TxCustomOutline.GetDataItem(Value: Pointer): Longint;
begin
  Result := FRootNode.GetDataItem(Value);
end;

function TxCustomOutline.GetItem(X, Y: Integer): LongInt;
var
  Value: TxGridCoord;
begin
  Result := -1;
...
function TxCustomOutline.GetTextItem(const Value: string): Longint;
begin
  Result := FRootNode.GetTextItem(Value);
end;

procedure TxCustomOutline.SetCurItem(Value: LongInt);
begin
...
procedure TxCustomOutline.SetOutlineStyle(Value: TOutlineStyle);
begin
...
procedure TxCustomOutline.SetDisplayWidth(Value: Integer);
begin
  FSettingWidth := True;
  try
...
function TxCustomOutline.GetNodeDisplayWidth(Node: TOutlineNode): Integer;
var
  Delta: Integer;
  TextLength: Integer;
begin
...
function TxCustomOutline.GetVisibleNode(Index: LongInt): TOutlineNode;
begin
  Result := FRootNode.GetVisibleNode(Index + 1);
end;

procedure TxCustomOutline.DrawCell(ACol, ARow: Longint; ARect: TRect; AState: TxGridDrawState);
var
  Node: TOutlineNode;
  Expanded: Boolean;
  HasChildren: Boolean;
...
procedure TxCustomOutline.DrawTree(ARect: TRect; Node: TOutlineNode);
var
  Offset: Word;
  Height: Word;
  OldPen: TPen;
...
procedure TxCustomOutline.DrawPictures(const BitMaps: array of TBitmap; ARect: TRect);
var
  I: Word;
  Rect: TRect;
  Value: TBitmap;
...
procedure TxCustomOutline.DrawText(Node: TOutlineNode; Rect: TRect);
begin
...
function TxCustomOutline.StoreBitmap(Index: Integer): Boolean;
begin
  Result := TOutlineBitmap(Index) in FUserBitmaps;
end;

procedure TxCustomOutline.ClearBitmap(var Bitmap: TBitmap; Kind: TOutlineBitmap);
begin
  if Bitmap <> nil then
  begin
    Bitmap.Free;
...
procedure TxCustomOutline.ChangeBitmap(Value: TBitmap; Kind: TOutlineBitmap);
var
  Bitmap: ^TBitmap;
begin
...
procedure TxCustomOutline.SetPicture(Index: Integer; Value: TBitmap);
begin
  ChangeBitmap(Value, TOutlineBitmap(Index));
end;

function TxCustomOutline.GetPicture(Index: Integer): TBitmap;
begin
...
procedure TxCustomOutline.LoadFromFile(const FileName: string);
var
  Stream: TStream;
begin
...
{procedure TxCustomOutline.SetMaskColor(Value: TColor);
begin
  FMaskColor := Value;
  Invalidate;
end;}

procedure TxCustomOutline.SetItemHeight(Value: Integer);
begin
  FItemHeight := Value;
...
procedure TxCustomOutline.SetStyle(Value: TOutlineType);
begin
  if Style <> Value then
  begin
    FStyle := Value;
...
procedure TxCustomOutline.SetOutlineOptions(Value: TOutlineOptions);
begin
...
procedure TxCustomOutline.LoadFromStream(Stream: TStream);
const
  EOF = Chr($1A);
  BufSize = 4096;
var
...
begin
  ...
  begin
    Font.Name := IniFile.FormsFontName;
    Font.Height := IniFile.FormsFontHeight;
  end;
end;
...
procedure TxCustomOutline.SaveToStream(Stream: TStream);
const
  BufSize = 4096;
var
  Buffer: PChar;
...
procedure TxCustomOutline.SetStrings(Value: TStrings);
begin
...
function TxCustomOutline.GetStrings: TStrings;
begin
  Result := FStrings;
end;

procedure TxCustomOutline.Error(const ErrorString: string);
begin
  Raise EOutlineError.Create(ErrorString);
end;

procedure TxCustomOutline.Expand(Index: LongInt);
begin
  if Assigned(FOnExpand) then FOnExpand(Self, Index);
end;

procedure TxCustomOutline.Collapse(Index: LongInt);
begin
