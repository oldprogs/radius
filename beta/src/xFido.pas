...
{-- FTS1}
  ...
  TEMSISeq = (es_INQ, es_REQ, es_CLI, es_HBT, es_DAT, es_ACK, es_NAK,
              ...
              es_TZP, es_PZT, es_None, es_DatError{$IFDEF EXTREME}, es_RCC{$ENDIF}, es_NOC);
...
//    Answering system options:
  ...
  TFidoAddress = record
  case Integer of
    0 : (Zone, Net, Node, Point: Integer; Domain: string[8]{; Handle: THandle});
    1 : (arr: array[1..4] of Integer; str: array [0..8] of char{; int: THandle});
  end;
  ...
  TNodePrefixFlag = (nfUndef, nfNormal, nfPoint, nfHub, nfNet, nfZone, nfPvt, nfHold, nfDown, nfOver, nfUrec, nfRegion);
...
type
  ...
  public
    ...
    procedure FillNodelist(const AAddr: TFidoAddress; const AStr: ShortString; APrefixFlag: TNodePrefixFlag);
...
const
  ...
  SProdCodes : array[0..MaxProductCode] of string = (
    ...
    'Xenia',                         {010B, Msdos/Os2, Mailer, Arjen_Lentz}
    'GMS',                           {010C, AmigaOS, Mailer, Mirko_Viviani}
    'HNET',                          {010D, Msdos, ???, Pedro_Jaramillo}
    'Shotgun_Professional',          {010E, Msdos, ???, Brent_Shellenberg}
    'SLIPgate',                      {010F, Msdos, ???, Kieran_Morrissey}
    'BBBS',                          {0110, MSDOS/OS2/NT/Amiga/Unix, Mailer/Packer, Kim_Heino}
    'NewsGate'                       {0111, Windows/NT, Packer/Gateway, Leilo_denna_Pietra}
//    'Radius'                         {15FF, Win32, Mailer, Denis_Voituk}
...
function NodeFSC62TimeEx(const Flags: string; const Addr: TFidoAddress; ALocal: Boolean): TFSC62Time;
...
uses SysUtils, xIP, LngTools, RRegExp, RadIni, Forms, Wizard, Recs,
     AltRecs, NdlUtil, Outbound, Classes;
...
begin
  ...
  if ((N.Addr.Net = N.Addr.Zone) and (N.Region = 0)) or (N.Addr.Net = 0) then Result := fntZone else
  if N.PrefixFlag = nfRegion {(N.Addr.Net<100)} then Result := fntRegion else Result := fntNet;
end;
