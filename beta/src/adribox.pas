...
uses
  ...
  StdCtrls, xFido, MClasses, ExtCtrls;
...
begin
  ...
  if addr.Zone <> 0 then begin
     A := InputFidoAddress(Caption, False, @Addr);
  end else begin
     A := InputFidoAddress(Caption, False, nil);
  end;
  case CollCount(A) of
    0 : ;
...
begin
  ...
  if OA <> nil then begin
     ...
     ParseAddress(lAddress.Text, Addr);
  end;
