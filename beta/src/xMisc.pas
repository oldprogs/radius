...
//    procedure SetRemoteVisible(v: boolean);
  ...
  TInetProtocol = class(TBiDirProtocol)
     ...
     constructor Create(ACP : TPort);
     destructor Destroy; override;
     function Decode_Basic(const inp: string): string;
     function Encode_Basic(const inp; Size: integer): string;
...
procedure PlaySnd(const s: string; Play: TTernarLogic);
...
uses SysUtils, NTdyn, wizard, plus, util, BleepInt, mmsystem, NdlUtil;
...
begin
  ...
  if ((Play = tlNotDefined) and (not Inifile.PlaySounds)) or (Play = tlFalse) then exit;
  if not IniFile.ReadBool('Sounds', 'c_' + s) then exit;
  p := Inifile.ReadString('Sounds', s);
  if p = s then
  begin
    p := MakeFullDir(inifile.HomeDir, 'snd');
    p := MakeFullDir(p, s + '.WAV');
  end;
  if ExistFile(p) then begin
    ext := UpperCase (ExtractFileExt (p));
    if ext = '.WAV' then
      sndPlaySound(PChar(p), SND_ASYNC)
    else if ext = '.MID' then begin
      e := mciSendString(PAnsiChar('open sequencer!' + p + ' alias snd'), nil, 0, 0);
      if e = 0 then
      begin
...
begin
  ...
  begin
    ...
    if Actually <> 0 then
    begin
      if Actually > PortInBufSize - CP.SzReadB then
      begin
...
begin
  ...
  j := 0;
  while (not SetCommTimeouts(FHandle, T)) and (FHandle <> INVALID_HANDLE_VALUE) do
  begin
    Inc(j);
    if j = 10 then begin
       e := GetLastError;
...
begin
   ...
   if (length(t) > 0) and (Collector[length(Collector)] <> #10) then
   begin
     loglastline := true;
     dec(wc);
   end else
...
begin
  ...
  if not Chat.Visible then Chat.Visible := true;
  Chat.Caption := str;
  ChatClosed := false;
  ChatOpened := True;
end;
...
function TInetProtocol.Decode_Basic;
var
  count,
  pos1,
  pos2: integer;
...
begin
  ...
  fillchar(out[1], length(s) * 3 div 4 + 3, #0);
  ...
  while pos2 <= count do begin
     if (pos1 > length(s)) or (s[pos1] = '\')  then begin
...
function TInetProtocol.Encode_Basic(const Inp; Size: integer): string;
var
...
begin
  ...
  for i := 1 to pos2 do out[i] := bin2b64[ord(out[i]) + 1];
...
begin
   ...
   if self.ID in [piSMTP, piPOP3, piGATE] then begin
      CP.PutChar(13);
   end;
   CP.PutChar(10);
   CP.Flsh;
...
begin
   ...
   if z = 'END' then begin
      ...
      fLine := '';
      case fCode of
   cB64: fLine := Decode_Basic(Trim(a));
   cUNK,
