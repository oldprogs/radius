...
{$ENDIF}
            ...
            PollOnDirect: Boolean;
            PollOnNormal: Boolean;
            RetryTimeoutType: Integer;
            UseSpace: boolean;
            PollAddFlags: string;
            NoHTML: boolean;
            GridInBWZ: boolean;
...
{$ENDIF}
            ...
            FormsFontHeight: integer;
            LoggerFontName: string;
            LoggerFontAttr: string;
            LoggerFontHeight: integer;
...
{            SoundsCollName: TStringColl;
            SoundsCollPath: TStringColl;
}
            SplitA,
            SplitB: integer;
...
begin
  ...
  begin
    try
      Result := ReadBool(Sect, Line, false);
    finally
      Free;
    end;
  end;
...
begin
  ...
  begin
    if Pos(UpperCase(Line), UpperCase(Result[i])) = 0 then
    begin
      Result.Delete(i);
    end;
  end;
...
begin
  ...
  begin
    try
      ...
      for i := 1 to s.Count do
      begin
        if pos(g.Name, s[i - 1]) > 0 then
        begin
          g.AddLine;
          t := ReadString('Grids', s[i - 1], '');
          for n := 1 to WordCount(t, ['|']) do
          begin
            c := ExtractWord(n, t, ['|']);
            if c = '-' then c := '';
...
begin
  ...
  begin
    s := '';
    for n := g.FixedCols to g.ColCount - 1 do
    begin
      c := g.Cells[n, i];
      if c = '' then c := '-';
      s := s + c + '|';
...
{  SoundsCollName := TStringColl.Create;
  SoundsCollPath := TStringColl.Create;}
...
{  FreeObject(SoundsCollName);
  FreeObject(SoundsCollPath);}
  if StringsList <> nil then
  begin
...
{$ENDIF}
     ...
     {$ENDIF}
      ...
      PollOnDirect := ReadBool(polls, 'PollOnDirect', true);
      PollOnNormal := ReadBool(polls, 'PollOnNormal', true);
      RetryTimeoutType := ReadInteger(polls, 'RetryTimeoutType', 1);
      UseSpace := ReadBool(polls, 'UseSpace', true);
      PollAddFlags := ReadString(polls, 'Flags', '1011');
      if length(PollAddFlags) < 4 then
        PollAddFlags := PadCh(PollAddFlags,'0',4-length(PollAddFlags));
...
{$IFDEF RASDIAL}
      ...
      iUserName := ReadString(_RasDial, 'UserName', '');
      iPassword := DecodeStr(ReadString(_RasDial, 'Password', ''));
      rasConnFlag := ReadBool(_RasDial, 'RasDialConnFlag', false);
      rasDisconnFlag := ReadBool(_RasDial, 'RasDialDisconnFlag', false);
{$ENDIF}
...
{      if SoundsCollName.Count <> 0 then
      begin
        SoundsCollName.FreeAll;
        SoundsCollPath.FreeAll;
      end;}

{      l := TStringList.Create;
      ReadSection(Sounds, l);
      for i := 0 to l.Count - 1 do
      begin
        s := l[i];
        SoundsCollName.Add(s);
        if lowercase(copy(s, 1, 2)) = 'c_' then s := ReadString(Sounds, s, '1')
        else s := ReadString(Sounds, s, s);
        SoundsCollPath.Add(s);
      end;
      l.Free;}
      ...
      LastIn := ReadString(Store, 'LastIn', 'Last  in: -:---/---@--------, --/--/-- --:--:--');
      LastOut := ReadString(Store, 'LastOut', 'Last out: -:---/---@--------, --/--/-- --:--:--');
...
//      WriteBool(main, 'ExtBanner', ExtBanner);
     ...
     {$ENDIF} 
      ...
      WriteBool(polls, 'PollOnDirect', PollOnDirect);
      WriteBool(polls, 'PollOnNormal', PollOnNormal);
      WriteInteger(polls, 'RetryTimeoutType', RetryTimeoutType);
      WriteBool(polls, 'UseSpace', UseSpace);
      ...
      WriteInteger(_interface, 'FormsFontHeight', FormsFontHeight);
      WriteString(_interface, 'FormsFontAttr', FormsFontAttr);
      WriteString(_interface, 'LoggerFontName', LoggerFontName);
      WriteInteger(_interface, 'LoggerFontSize', LoggerFontHeight);
...
{$IFDEF RASDIAL}
      ...
      WriteString(_RasDial, 'UserName', iUserName);
      WriteString(_RasDial, 'Password', EncodeStr(iPassword));
      WriteBool(_RasDial, 'RasDialConnFlag', rasConnFlag);
      WriteBool(_RasDial, 'RasDialDisconnFlag', rasDisconnFlag);
{$ENDIF}
...
{Store}
      WriteString(Store, 'LastIn', LastIn);
      WriteString(Store, 'LastOut', LastOut);
{end}
...
{sounds}
      if SoundsCollName.Count > 0 then
        for i := 0 to SoundsCollName.Count - 1 do
        begin
          WriteString(Sounds, SoundsCollName[i] , s)
        end;
{end}
*)
...
begin
  ...
  begin
    ...
    if MatchMaskAddressListSingle(a, s) then
    begin
      Result := i;
      Break;
    end;
