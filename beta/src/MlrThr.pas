...
{$ENDIF}
  ...
  PanelOwnerInfo   = Pointer($FFFFFFFA);
...
{$ENDIF}
  ...
  TPollType = (ptpUnknown, ptpOutb, ptpCron, ptpManual, ptpImm, ptpBack{$IFDEF EXTREME}, ptpRCC{$ENDIF});
...
{3}
    ...
    function DoPollsRecalc (var PubInst: Boolean): Boolean;
    procedure LetsSleep;
    procedure DoEvt;
...
uses
   ...
   {$IFDEF EXTREME}
   p_RCC, p_FTP, p_HTTP, p_SMTP, p_POP3, p_GATE,
   {$ENDIF}
   ...
   {$ENDIF}
   ;
{procedure ToMonitor(Ind:String;Mess:String);
var
  h     : Hwnd;
  ds    : TCOPYDATASTRUCT;
  params: String;
begin
  h:=FindWindow(nil,'Radius monitor');
  if h=0 then exit;
  params:=Ind+Mess;
  ds.dwData := 0;
  ds.cbData := length(params) + 1;
  ds.lpData := PChar(Params);
  SendMessage(h, WM_COPYDATA, Application.MainForm.Handle, integer(@ds));
end;
}
...
begin
  ...
  if not ShowBalloonTT then b := false;
{   if (MailerThreads <> nil) and (MailerThreads.Count <= 1) then
   begin
      ...
      begin
         ...
         begin
            if (Application.MainForm as TMailerForm).TrayIcon.Hint <> 'Radius: ' + S then
            begin
               if b then
               begin
                  (Application.MainForm as TMailerForm).TrayIcon.ShowHint('Radius Info', S);
               end;
               (Application.MainForm as TMailerForm).TrayIcon.Hint := 'Radius: ' + S;
            end;
         end;
      end;
   end
   ...
   begin
      if (MailerThreads <> nil) then Application.Title := Format('Radius: %d lines', [MailerThreads.Count]);
      if Application.MainForm <> nil then
      begin
         if (Application.MainForm as TMailerForm).TrayIcon <> nil then
         begin
  ...
  ss := '';
  MailerThreads.Enter;
  for i := 0 to CollMax(MailerThreads) do
  begin
    m := MailerThreads[i];
    ss := ss + m.Name;
    ss := ss + ' : ' + m.StatusMsg + #13#10;
{    if TimerInstalled(m.D.TmrPublic) then
    begin
      r := 'remaining time: ' + IntToStr(RemainingTimeSecs(m.D.TmrPublic));
    end else
    begin
      if m.FstTic <> 0 then
      begin
        o := GetTickCount - m.fsttic;
        r := 'time elapsed: ' + FormatDateTime('hh:mm:ss', EncodeTime(((o div 1000) div 60) div 60, ((o div 1000) div 60) mod 60, (o div 1000) mod 60, o mod 1000));
      end else
      begin
        r := '';
      end;
  ...
  delete(ss,length(ss)-1,2);
      ...
      Application.Title := 'Radius lines: ' + IntToStr(MailerThreads.Count);
      if Application.MainForm <> nil then
      begin
         if (Application.MainForm as TMailerForm).TrayIcon <> nil then
         begin
            ...
            if (Application.MainForm as TMailerForm).TrayIcon.Hint <> ss then
            begin
               (Application.MainForm as TMailerForm).TrayIcon.Hint := ss;
            end;
         end;
      end;
end;
...
var
  ...
  begin
    Result := plsN_A;
    if (p.Owner = PollOwnerExtApp) and (not PubInst) then
      p.Owner := nil;
    if p.Owner <> nil then
    begin
      if SC <> nil then
...
{$ELSE}, {$ENDIF}
    ...
    begin
      ...
      begin
        if not TimerInstalled(p.LastTry) then
          NewTimerSecs(p.LastTry, inifile.FPFlags.Standoff * 60);
        if p.FileSendDelayed then
        begin
    ...
    end else if (p.FileSendDelayed) then
      p.FileSendDelayed := False; //bugfix by Denis Voituk
    Result := plsFRB;
...
function GetAvailPoll(AOwnPolls: TColl; var PubInst: Boolean; Mlr: TMailerThread; var AP: TFidoPoll; ALogger: TAbstractLogger; ZMHactive: Boolean): Boolean;
...
begin
  ...
  begin
    p := FidoPolls[i];
    p.State := GetPollState(AOwnPolls, P, PubInst, Mlr, SC, TQ);
...
{          end;}
      ...
      plsBSY:
        begin
          ...
          begin
            ...
            if (p.Typ = ptpManual) or (p.Typ = ptpImm) or
            {$IFDEF EXTREME}
              (p.Typ = ptpRCC) or
            {$ENDIF}
              (p.Typ = ptpBack) then
      ...
      plsAVL :
        if not ZMHactive or
          (osImmedMail in p.Flav) or
          (osCrashMail in p.Flav) or
          (osDirectMail in P.Flav) or
          (osNormalMail in P.Flav) then
        begin
          AP := p;
          PubInst := False;
          Result := True;
...
var
  ...
  I: Integer;
const
  CTyp: array[TPollType] of string = ('???', 'Outb', 'Cron', 'Manual', 'Outb/Imm', 'Callback'{$IFDEF EXTREME}, 'Mirror'{$ENDIF});
begin
  EnterFidoPolls;
  AP := nil;
  for I := 0 to FidoPolls.Count - 1 do
...
function OutDial(A: TOutStatusSet; PollOnDir: Boolean; PollOnNorm: Boolean): Boolean;
begin
   Result := (osImmed in A) or
      ...
      (osCrashMail in A) or
      (osCallback in A);
   if (not Result) and PollOnDir then
      Result := (osDirect in A) or
         (osDirectMail in A);
   if (not Result) and PollOnNorm then
...
begin
//  EnterFidoPolls;
  PollOnDir := IniFile.PollOnDirect; //pofPollOnDir in FidoPolls.Options.d.Flags;
  PollOnNorm := IniFile.PollOnNormal;
//  LeaveFidoPolls;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ((p.Typ = ptpOutb) or
            (p.Typ = ptpImm)) and (not OutDial(n.FStatus, PollOnDir, PollOnNorm)) then FreePoll_I_P;
        c.AtFree(j);
      end else
      begin
  ...
  begin
    n := c[i];
    if OutDial(n.FStatus, PollOnDir, PollOnNorm) then
    begin
      ...
      begin
        if (osImmedMail in n.FStatus) or (osImmed in n.FStatus) then
...
{$IFDEF WS}
  ...
  {$IFDEF WS}if not m.DialupLine then m.OwnPolls := TColl.Create else {$ENDIF}
  begin
...
constructor TMlrEvtShutdownTerminate.Create;
begin
  inherited Create;
end;
...
begin
  ...
  begin
    T.State := msCheckOut;
//    T.msCheckOut_From := 'procedure TMlrEvtPollsRecalc.Execute(T: TMailerThread);';
...
{$ENDIF}
  ...
  {$IFDEF USE_TAPI}
  if (CP <> nil) and (CP is TTAPIPort) then
  begin
    (CP as TTAPIPort).Messages.Enter;
    while (CP as TTAPIPort).Messages.Count > 0 do
    begin
      if IniFile.TAPIDebug then
      begin
    ...
    while (CP as TTAPIPort).TAPILog.Count > 0 do
    begin
      if not IniFile.TAPIDebug then
      begin
         ...
         case CC of
            ...
            '|':
               begin
                  ...
                  CP.Flsh;
//                  NewTimerSecs(DT, 10);
                  NewTimerSecs(DT, 3);  //Argus default!
                  repeat
                     DoAccumulate;
                     ...
                     if Application.MainForm = nil then Terminated := True;
                     if Terminated or (ToSleep = 0) then
                     begin
                       Result := False;
                       Exit;
                     end;
...
begin
   ...
   begin
      ...
      if (IniFile.DynamicRouting) and
         (SD.PasswordProtected or (SD.ActivePoll <> nil)) then
      NetmailHolder.Route(SD.rmtAddrs[n], Log);
   end;
   CfgLeave;
   N := 0;
   ...
   begin
      ...
      if (SD.txMail = 0) and (SD.txFiles = 0) then
      ...
      if ProtCore in [ptBinkP{$IFDEF EXTREME}, ptPOP3, ptGATE{$ENDIF}] then SD.Prot.ReportTraf(SD.txMail, SD.txFiles);
   end;
   Ok := True;
   SD.OutFiles.Enter;
   SD.OutFiles.PurgeDuplicates(SD.TxMail, SD.TxFiles);
...
const
   Cores: array[TSessionCore] of string = ('???', 'FTS-0001', 'EMSI', 'BinkP'{$IFDEF EXTREME}, 'FTP', 'HTTP', 'SMTP', 'POP3', 'GATE'{$ENDIF});
...
{$ENDIF}
   ...
   FreeObject(P.T.Stream);
   {$IFDEF EXTREME}
   if (SD.SessionCore in [scFTP, scPOP3, scGATE]) and P.DummyNextFile then begin
      ScanOut(True);
...
{         if s = 'Reactor' then // Ётим ты ничего не добьешьс€, кроме неудобств своим пользовател€м.
         begin
           Log(ltGlobalErr, 'Reactor violates Radius license. Please replace mailer at the remote.');
           exit;
         end;}
...
//   SD.NeedModemStatx := True;
   ...
   if (SD.SessionCore = scEmsiWz) or
     ((DS.rmtPhone <> '') {$IFDEF EXTREME} and not (SD.SessionCore in [scFTP, scHTTP]){$ENDIF}) then
   ...
   //
   LogTrafInfo;
   if (SD.PostEMSILogErrors <> nil) {$IFDEF EXTREME} and not (SD.SessionCore in [scFTP, scHTTP]) {$ENDIF} then
   begin
...
begin
   ...
   SessionIsOn := True;
   if not (SD.SessionCore in [scBinkP{$IFDEF EXTREME}, scFTP, scHTTP, scSMTP, scPOP3, scGATE{$ENDIF}]) and (SD.ActivePoll = nil) then begin
...
begin
  ...
  {$ENDIF}
   ...
   if not (ProtCore in [ptBinkP{$IFDEF EXTREME}, ptFTP, ptHTTP, ptSMTP, ptPOP3, ptGATE{$ENDIF}]) then
   begin
      if SD.SessionKey = 0 then
      begin
         SD.MayEMSI := True;
...
begin
  ...
  //“ут можно попробовать показать, с кем сесси€
  //TInf.Mesage:=Addr2Str(SD.ActivePoll.Node.Addr);
  //TInf.Show;
//  OutMgrThread.Enter;
...
{$ENDIF}
      ...
      {$IFDEF EXTREME}
            if SD.Prot.ID in [piRCC, piFTP, piHTTP, piSMTP, piPOP3, piGATE] then
...
var
   ...
   begin
      EnvStr := EnvStr + Format('RADIUS_%s=%s'#0, [s1, s2])
   end;
  ...
  case State of
    msExtApp_0:
       begin
         ...
         if Prefix = 'TELNET:' then State := msRedir_0
         else
         {$ENDIF}
             State := msExtApp_1;
       end;
  ...
  case State of
     msRedir_0:
       begin
         ...
         if ConnectToRemoteHost(RemoteSocket,'192.168.16.197',21) = 0 then
           State := msRedir_1
         else
           State := msRedir_2;
       end;
...
//               Buf[i]:=byte(SD.InB[i+1]);
         ...
         BufLen := 2048;
         sss := ReadFromRemoteHost(RemoteSocket, Buf, BufLen, @ReadOL);
         if sss = 0 then
         begin
           if BufLen > 0 then
...
{$ENDIF}
      ...
      msCN_HandshakeOK:
         begin
           redirout := (CP is TSockPort) and TSockPort(CP).Outgoing;
           if (SD.ActivePoll <> nil) or redirout then
              State := msHSh_s1
           else
              State := msHSh_r1;
         end;
...
{$ENDIF WS}
   ...
   begin
      ...
      Result := False;
      State := msCheckOut;
//      msCheckOut_From := 'procedure TMailerThread.DoMisc; #1';
      ...
      try
         ...
         repeat
            ...
            if c = '%STOP' then begin
               ...
               Result := True;
               State := msCheckOut;
//               msCheckOut_From := 'procedure TMailerThread.DoMisc; #2';
               exit;
            end else
...
var
   R, TD: DWORD;
   c, i, AvlPollsCount: integer;
   s: string;
   p: TFidoPoll;
   PubInst, PollsCalced: Boolean;
begin
   PollsCalced := False;
   UpdateModem;
   case State of
      msCheckOut:
         begin
            PubInst := False;
            PollsCalced := DoPollsRecalc (PubInst);
         end;
      msError:
         begin
...
{$ENDIF}
      ...
      msIdleA_Expired:
         begin
           ClearTmr1;
//           msCheckOut_From := 'msIdleA_Expired #1';
           ...
           begin
             ...
             PollsCalced := DoPollsRecalc (PubInst);
             R := EP.DwordValueD(eiRetry, inifile.FPFlags.Retry);
             case IniFile.RetryTimeoutType of
               0: NewTimerSecs(TmrNextDial, R);
               1: NewTimerAvg(TmrNextDial, R);
               2:
               begin
                AvlPollsCount := 0;
                for i := 0 to FidoPolls.Count - 1 do
                begin
                  p := FidoPolls[i];
                  if (p.State = plsAVL) or (p.State = plsPUB) then
                    Inc (AvlPollsCount);
                end;
                if R < AvlPollsCount then
                  R := 1
                else if AvlPollsCount > 1 then
                  R := R div AvlPollsCount;
                NewTimerSecs(TmrNextDial, R);
               end;
             end;
//             msCheckOut_From := 'msIdleA_Expired #2';
      ...
      msIdle: // waiting for a call
         begin
           ...
           {$ENDIF}
           ShowIt('Idle', false);
           if not TimerInstalled(SD.TmrReInit) then NewTimerAvg(SD.TmrReInit, CReinitTime)
           else
             if not TimerExpired(SD.TmrReInit) then DoIdle
             else begin
           ...
           {$ELSE}
           if EP.VoidFound(eiRASEvent) then
             FidoPollsLog('RAS event found, but RAS is not available in this version. Dropping Event');
           {$ENDIF}
         end;
{$IFDEF USE_TAPI}
      msStartIdle_TAPI:
...
//            State := msRescan;  //„итайте ru.argus.devel
   ...
   if PubInst then
     if TimerInstalled(D.TmrPublic) then
        ...
        GlobalFail('%s', ['TMailerThread.DoMisc/TimerInstalled'])
   else if PollsCalced then
   begin
     ClearTmrPublic;
     ClearTimer(TmrNextDial);
   end;

   if EP.VoidFound(eiSNDEvent) then
      if EP.BoolValueD(eiSNDEvent, False) then
        SoundsON := tlTrue
      else
        SoundsON := tlFalse
   else
...
begin
   ...
   if EP.VoidFound(eiSNDEvent) then
   begin
     case EP.BoolValueD(eiSNDEvent, False) of
       false: SoundsON := tlFalse;
       else SoundsON := tlTrue;
     end; {of case}
   end else
   begin
     SoundsON := tlNotDefined
   end;
...
function TMailerThread.DoPollsRecalc (var PubInst: Boolean): Boolean;
var
  c: TOutNodeColl;
  p: TFidoPoll;
begin
//  try
//    if msCheckOut_From = '' then;
    State := msStartIdle;
    ...
    if GetAvailPoll(OwnPolls, PubInst, Self, p, Logger, EP.VoidFound(eiZMHEvent)) then
    begin
      if SD.ActivePoll <> nil then SD.ActivePoll.Release;
{      if EP.VoidFound(eiZMHEvent) then
      begin
        if not (osImmedMail in p.Flav) and
           not (osCrashMail in p.Flav) and
           not (osDirectMail in P.Flav) and
           not (osNormalMail in P.Flav) then
        begin
          ClearTmrPublic;
          ClearTimer(TmrNextDial);
          SetStatusMsg(rsMMIdle, '');
      ...
      if (State = msStartIdle) or (State = msIdle) then
      begin
         SD.ActivePoll := p;
         State := msStartDial;
      end;
    ...
    begin
      LeaveFidoPolls;
{      if (p <> nil) and EP.VoidFound(eiZMHEvent) then
      begin
        if not (osImmedMail in p.Flav) and
           not (osCrashMail in p.Flav) and
           not (osDirectMail in P.Flav) and
...
begin
   ...
   ToSleep := CheckEvents;
   if ToSleep = 0 then ToSleep := 20000
   else ToSleep := MinD(20000, ToSleep);
...
{$IFDEF RASDIAL}
      ...
      begin
         Inc(oc.Age);
         for n := 0 to oc.Atoms.Count - 1 do
         begin
            a := TEventAtom(oc.Atoms[n]);
            if (a.Typ = eiRASEvent) and (oc.Age > oc.Len) then
            begin
               RASInvoked := False;
               RASHangup := False;
               if RASprocess then
...
begin
  ...
  begin
    ...
    repeat
      ...
      Inc(pc);
      PubInst := False;
      if (OutConnsAvail > 0) and (not Terminated) and (GetAvailPoll(OwnPolls, PubInst, PollOwnerDaemon, p, Logger, False)) then
      begin
        BurnOutLine(p);
        EnterFidoPolls;
        Continue;
...
begin
  ...
  if (not ParsePortsList(@D.Ifcico, Cfg.IPData.InPorts.Ifcico)) or
      ...
      (not ParsePortsList(@D.BinkP, Cfg.IPData.InPorts.BinkP))
   {$IFDEF EXTREME}
      or
      ...
      (not ParsePortsList(@D.GATE, Cfg.IPData.InPorts.GATE))
   {$ENDIF}
...
begin
   ...
   if IniFile.accessFName = '' then
     CommonLog.accessFName := ''
   else
     CommonLog.accessFName := MakeNormName(dLog, IniFile.accessFName);
   if IniFile.agentFName = '' then
     CommonLog.agentFName := ''
   else
     CommonLog.agentFName := MakeNormName(dLog, IniFile.agentFName);
   if IniFile.StatxFName = '' then
     CommonStatxFName := ''
   else
...
{  repeat
    WaitEvt(oEvt, 10000);
  until (not ApplicationDowned) or (Terminated) or (handupdate);
}
...
{  for i := 0 to MailerThreads.Count - 1 do
  ...
  for i := 0 to MailerThreads.Count-1 do
    if not TMailerThread(MailerThreads[i]).DialupLine then
    begin
      E := TMlrEvtShutdownTerminate.Create;
      TMailerThread(MailerThreads[i]).InsertEvt(E);
    end;
  for i := 0 to MailerThreads.Count-1 do
    if not TMailerThread(MailerThreads[i]).DialupLine then
