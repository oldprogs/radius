...
begin
  if UpperCase(copy(InMsg, 1, 3)) = 'CHT' then begin
     ...
     if {ChatOpened and }(UpperCase(s) = 'BYE'){ and ChatClosed} then
     begin
       ChatOpened := False;
...
begin
  ...
  if CustomInfo = 'CRYPT' then
  begin
    if RemoteCanCrypt then
    begin
      if not RemoteCanCryp then
      begin
        RemoteCanCryp := True;
...
begin
  if (ChatBuf <> '') and RemoteCanChat then
  begin
  ...
  begin
    ...
    if not Chat.Visible and ChatOpened then
    begin
      ChatBuf := 'BYE';
      ChatOpened := False;
      ChatClosed := True;
