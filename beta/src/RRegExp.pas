...
implementation uses xBase;//, crtl;
...
begin
  Result := 0;
  if poAnchored in FPattOptions then
    Inc(Result, PCRE_ANCHORED);
  if poCaseless in FPattOptions then
    Inc(Result, PCRE_CASELESS);
  if poDollarEndOnly in FPattOptions then
    Inc(Result, PCRE_DOLLAR_ENDONLY);
  if poDotAll in FPattOptions then
    Inc(Result, PCRE_DOTALL);
  if poExtended in FPattOptions then
    Inc(Result, PCRE_EXTENDED);
  if poMultiline in FPattOptions then
    Inc(Result, PCRE_MULTILINE);
  if poExtra in FPattOptions then
    Inc(Result, PCRE_EXTRA);
  if poUngreedy in FPattOptions then
    Inc(Result, PCRE_UNGREEDY);
end;
