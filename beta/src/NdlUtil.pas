...
begin
  ...
  begin
    ...
    ZC.MemPos := -1;
    if (not ReadFile(Stream.Handle, ZC.ZoneData, SizeOf(TFidoZoneData), Actually, nil)) or (SizeOf(TFidoZoneData) <> Actually) then
    begin
      ZC.Free;
      Exit;
    end;
...
begin
  ...
  FreeObject(ZonesBin);
  FreeObject(Stream);
  if PhantomNodes = nil then PhantomNodes := TColl.Create('PhantomNodes');
...
var
  ...
  begin
    ...
    if p.Addr.net = 0 then
      ...
      P.Region := CurRegion;
    if Flag = nfPoint then Inc(CompiledPoints)
    else Inc(CompiledNodes);
  ...
  begin
    Addr.Point := 0;
    if (IniFile.MainReg = 0) and (CompareAddrs(Addr, IniFile.MainAddr) = 0) then
    begin
  ...
  begin
    ...
    begin
      if (TFidoZone(Zones.At(i)).d.Domain = Addr.Domain) and
         ...
         (TFidoZone(Zones.At(i)).d.Net = Addr.Net) then
          CurRegion := TFidoZone(Zones.At(i)).d.Region;
    end;
    Point := True;
  end;
  ...
  if St = INVALID_HANDLE_VALUE then Error := SysErrorMessage(GetLastError) else
    begin
      ...
      // differentiate: pointlists first, then nodlist
        ...
        if not CompileFile(Cfg.Nodelist.Files[I], _domain, true) then
        begin
          ndls := ndls + char(I);
        end;
      end;
      if ndls <> '' then
        while length(ndls) > 0 do
        begin
          if (Cfg.Nodelist.Files.Count = AltCfg.NodelistDataDomain.Count) and
...
//    windlgt('You should recompile your nodelists');
  ...
  if NodeController = nil then Result := nil
  else Result := NodeController.SearchNode(Addr);
  LeaveNlCS;
end;
...
{  fs := 'IP,TCP,INA'; agradov}
  ...
  if g <> '' then ga := g;
  CfgEnter;
  for i := 0 to MinI(CollMax(Cfg.IpDomA), CollMax(AltCfg.IpDomC)) do
  begin
    s := Cfg.IpDomA[i];
    if MatchMaskAddressListSingle(Addr, s) then
    begin
...
begin
  ...
  if (n <> nil) and (n.Addr.Point = 0) then
  begin
    s1 := n.Phone;
...
{  AdvNodeCache.Enter;
  ...
  AdvNodeCache.Leave;
  if f then Exit;} // was in Argus. To find out why it doesn't work. Do not delete please.
...
//    windlgt('You should recompile your nodelists');
  ...
  if (IniFile.MainReg = 0) and not NodelistMissed then
  begin
