...
type
  ...
  TxGridMouseState = {set of }(ssRightDown,  ssRightUp);
  ...
  private
    ...
    procedure DrawMove;
    procedure GridRectToScreenRect(const GridRect: TxGridRect;
      var ScreenRect: TRect; IncludeLine: Boolean);
    procedure HideEdit;
    procedure Initialize;
    procedure InvalidateGrid;
    procedure InvalidateRect(const ARect: TxGridRect);
//    procedure InvertRect(const Rect: TRect);
    ...
    procedure SetSelection(const Value: TxGridRect);
  ...
  TxRowEvent = procedure (Sender: TObject; xRow: LongInt) of object;
  ...
  private
    ...
    FEditUpdate: Integer;
    FPasswordCol: Integer;
    fOnRowInsert: TxRowEvent;
    fOnRowDelete: TxRowEvent;
    fCheckBoxes: boolean;
    fCheckl: TImageList;
...
//    procedure PM_Popup(Sender: TObject);
  ...
  protected
    ...
    function  GetChecked(i: longint): boolean;
    procedure SetChecked(i: longint; v: boolean);
  public
  ...
  public
    procedure ImportTable(const FileName: string; divider: char);
    property RowChecked[i: integer]: boolean read GetChecked write SetChecked;
  published
    ...
    property PasswordCol: Integer read FPasswordCol write FPasswordCol default -1;
    property OnRowInsert: TxRowEvent read fOnRowInsert write fOnRowInsert;
    property OnRowDelete: TxRowEvent read fOnRowDelete write fOnRowDelete;
 end;
...
begin
  ...
  begin
    ...
    Font.Name := 'FixedSys';
    Font.Height := IniFile.ReadInteger('interface', 'FormsFontHeight', -11);
  ...
  begin
    ...
    begin
      ...
      DrawLines(goFixedHorzLine in Options, goFixedVertLine in Options,
        0, 0, [0, 0, Horz.FixedBoundary, Vert.FixedBoundary], clWindowText, FixedColor);
      DrawLines(goFixedHorzLine in Options, goFixedVertLine in Options,
        LeftCol, 0, [Horz.FixedBoundary, 0, Horz.GridBoundary,
        Vert.FixedBoundary], clWindowText, FixedColor);
      DrawLines(goFixedHorzLine in Options, goFixedVertLine in Options,
        0, TopRow, [0, Vert.FixedBoundary, Horz.FixedBoundary,
        Vert.GridBoundary], clWindowText, FixedColor);
...
procedure TAdvCustomGrid.AdjustDefault;
...
begin
  ...
  DefaultRowHeight := MaxI(chDef, chFixed);
end;

procedure TAdvCustomGrid.GridRectToScreenRect(const GridRect: TxGridRect;
  var ScreenRect: TRect; IncludeLine: Boolean);
...
procedure TAdvCustomGrid.InvalidateRect(const ARect: TxGridRect);
var
  InvalidRect: TRect;
begin
  if not HandleAllocated then Exit;
...
var
  ...
  MoveDrawn: Boolean;
begin
  if ssRight in Shift then FMouseState := ssRightDown;
  MoveDrawn := False;
  HideEdit;
...
begin
  if not (ssRight in Shift) then FMouseState := ssRightUp;
  try
    case FGridState of
      gsSelecting:
        begin
...
procedure TAdvCustomGrid.SetSelection(const Value: TxGridRect);
var
  OldSel: TxGridRect;
begin
  OldSel := Selection;
...
begin
  if FMouseState = ssRightDown then exit;
  NewAnchor := FAnchor;
...
begin
   if v <> fCheckBoxes then begin
      fCheckboxes := v;
      Invalidate;
   end;
end;

function TAdvGrid.GetChecked;
begin
   Result := False;
   if fCheckBoxes then begin
      Result := Boolean(Objects[0, i]);
   end;
end;

procedure TAdvGrid.SetChecked;
begin
   if fCheckBoxes then begin
      Objects[0, i] := pointer(v);
   end;
end;
...
begin
  ...
  ClearRow(Row);
  if goDigitalRows in Options then RenumberRows;
  if Assigned(fOnRowInsert) then fOnRowInsert(Self, Row);
end;
...
begin
  ...
  if goDigitalRows in Options then RenumberRows;
  if not (csDesigning in ComponentState) then
  if Assigned(fOnRowInsert) then fOnRowInsert(Self, RowCount - 1);
end;
...
begin
  inherited CreateWnd;
  if not IgnoreAdjust then begin
     Canvas.Font := Font; chDef := CharHeight;
     Canvas.Font := FixedFont; chFixed := CharHeight;
