...
uses xBase, SysUtils, Windows, Classes, RasUnit, MClasses;
  ...
  TMyDialParam = Record
    AMsg : Integer;// - message code
    AState : TRasConnState;// - connection status (variable type is defined in  RasUnit.pas)
    AError : Integer;// - error code
  end;
...
uses RadIni, Forms, MlrThr, recs, xMisc;
...
begin
  ...
  Log.FTag := ltRasDial;
  Log.FMsg := 0;
  if Inifile.ras_log = '' then
    Log.FName := ''
  else
    Log.FName := MakeNormName(dlog, Inifile.ras_log);
  fConnected := csIdle;
...
{var
  RasStats: RASSTATS2000;}
begin
  if (fConnected = csConnecting) then exit;
{  if (GetActiveConnHandle(fEntryName) = 0) and (fConnected = csConnected) then Disconnect;}
  if (fConnected = csIdle) then
  begin
...
//  p:pointer;
  n: integer;
begin
  if (hRas <> 0) and HangUp then RasHangUpA(hRas);
