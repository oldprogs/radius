...
//wsock,
  MlrThr, MlrDefs,
{$ENDIF}
  SysUtils, RadIni, Recs, Crypt, xFido, Wizard, NdlUtil;
...
procedure __xclose(var h: TSocket);
var
  ls: TSocket;
  e: Integer;
begin
  ...
  begin
    ...
    begin
      e := WSAGetLastError;
      if e <> WSAENOTSOCK then GlobalFail('closesocket error %d (%s)', [e, WSAErrMsg(e)])
      else exit;
    end;
  end;
  Dec(SocksAlloc);
...
procedure __xshutdown(FHandle: DWORD);
var
  e: Integer;
begin
  if shutdown(FHandle, 2) = SOCKET_ERROR then
  begin
    ...
    begin
      e := WSAGetLastError;
      if e <> 10057 then GlobalFail('shutdown failed, error %d. Try using WinSockVersion registry variable', [e, WSAErrMsg(e)]);
    end;
  end;
end;
...
begin // procedure TSockListen.InvokeExec;
  ...
  begin
    case Typ of
      ptifcico, ptTelnet
      {$IFDEF EXTREME}, ptFTP, ptHTTP, ptPOP3, ptSMTP, ptGATE {$ENDIF}:
        SendStr('BUSY' + ccCR);
...
begin
  ...
  begin
    ...
    if Length(IniFile.ProxyUserName) <= 0 then IniFile.ProxyUserName := 'RADIUS';
  ...
  begin
{
  ...
  begin
    Dec(SockMgr.SocksAvail);
    Thr := OpenPort(FSocket, ptTelnet, FPort);
    //TSockPort.Create(FSocket, nil, FPort, ptTelnet{ptUndefined});
    Thr.Incoming := False;
    Thr.Outgoing := True;
    SockMgr.NewSocks.Insert(Thr);
    SockMgr.NewSocks.Leave;
...
{    if ParseAddress('111:222/333.444', addr) then
    begin
      an := FindNode(Addr);
