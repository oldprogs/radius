...
//  IniFName         = 'config.ini'; // visual
  ...
  HttpProxyCommand : array [TProxyType] of string = (
                   '',
                   '',
                   '',
                   'PUT HTTP://%s:%d HTTP/1.0'#13#10,
                   'CONNECT %s:%d HTTP/1.0'#13#10) ;
...
type
  ...
  TModemStdRespIdx = (mrpRing, mrpConnect, mrpOK, mrpBusy, mrpNoCarrier, mrpNoDial, mrpError, mrpVoice, mrpRinging, mrpNone{$IFDEF USE_TAPI}, mrpTapi{$ENDIF});
  ...
  TModemCmdsColl = class(TStringColl)
    ...
    property Test          : string index 6 read GetString;
  end;
...
//    InBandwidth, OutBandwidth: integer;
  ...
  TPasswordColl = class(TColl)
    AuxFile: string;
    function Password(const Addr: TFidoAddress): string;
    function Crc32Item(Item: Pointer; Crc32: DWORD): DWORD; override;
    constructor Load(Stream: TxStream); override;
    procedure Store(Stream: TxStream); override;
    function Crc32(Init: DWORD): DWORD; override;
  ...
  TPollOptionFlag = (pofHold, pofPollOnDir, pofPollOnNorm);
...
{ visual {
    ...
    CfgTxWindow,
    CfgRxWindow: integer;
    RunPostProcessorsAfterSuccessSessions: bool;
    CPS_MinBytes,
    CPS_MinSecs: integer;
    DontLogTariff,
    UseTrayIcon,
    ...
    MinInboundSpace,
    MinSecInboundSpace,
    MinTempInboundSpace: integer;
    MainPriority: integer;
    WatcherPriority: TThreadPriority;
    EnableWatcher: bool;
    HydraOemCharSet: bool;
 ...
 visual }
  ...
  TReqRec = class
    ...
    Files: TColl;
    SRPs: TStringColl;
    procedure Add(const FName, APsw: string; const Info: TFileInfo);
...
function NameCom(const s: string): Integer;
...
//{$IFDEF RASDIAL}
//function Ras_Log: string;
//{$ENDIF}

function OpenBWZLog: Boolean;
procedure CloseBWZLog;
function GetBWZ(const FName: string; FSize, FTime: DWORD; const Addr: TFidoAddress; AddrList: TFidoAddrColl): TBWZRec;
function AddBWZ(const FName: string; FSize, FTime, Prot: DWORD; const Addr: TFidoAddress): TBWZRec;
procedure FreeBWZ(var Rec: TBWZRec);
function ParseREQ(var ASC: TStringColl): TColl;
procedure ScanREQ(Coll: TColl);
function DialAllowed(AR: TRestrictionData; const Flags, Phone: string; const Addr: TFidoAddress; var AExpl: string): Boolean;
...
type
  ...
  dmZONE, dmNET, dmNODE, dmPOINT, dmHZONE, dmDZONE, dmHNET, dmHNODE, dmHPOINT,
  dmXZONE, dmXNET, dmXNODE, dmXPOINT, dmSTATUS, dmTSTATUS, dmDOMAIN);
...
begin
  Result := '';
  if IniFile = nil then exit;
  Result := IniFile.MainAddr.Domain;
end;
...
begin
  ...
  DstPfx := Stream.Get;
  Files  := Stream.Get;
  if (Files <> nil) and (Files.Count > 0) and (pos('|', Files[0]) = 1) then
  begin
     ...
     if TNodelistDataFixUp then Stream.Get;
  end;
...
begin
  ...
  PathNames.Fill(['GIN','SIN','TMP','OUT','LOG']);
  ParseAddress('0:0/0', F);
  IniFile.MainAddr := F;
end;
...
{$IFDEF EXTRIME}
  IPData.InPorts.Fill(['60179', '23 60177', '24554', '21', '80', '25', '110', '60180']);
{$ELSE}
  IPData.InPorts.Fill(['60179', '23 60177', '24554', '', '', '', '', '']);
{$ENDIF}
  IPData.StationData.Fill(['Radius IP Server',
    '1:2/3.4 5:6/7.8',
    'Mickey Mouse',
    'Universe, Earth',
    '127.0.0.1',
    'MO,CM,IBN,ITN,IFC,IFT,IHT']);

  IPData.InC := 16;
  IPData.OutC := 4;
  IPData.Speed := 600;
  IPData.Banner := 'Welcome to Radius IP Server'#13#10#13#10+
...
begin
  IpDomA := TIpDomCollA.Create('IpDomA');
  IpDomA.Add('1:*/* 2:*/* 3:*/* 4:*/* 5:*/* 6:*/* 7:*/*');
end;

procedure CreateIpDomCollB;
begin
  IpDomB := TIpDomCollB.Create('IpDomB');
...
begin
  ...
  if S1 = '' then
  begin
    S1 := MakeNormName(ExtractFilePath(WindowsDirectory), 'ARGUS');
    if not FileExists(MakeNormName(S1, 'config.bin')) then
    begin
      StoreConfigAfter := True;
      S1 := ExtractFileDir(ParamStr(0));
    end;
{    if not SetRegHomeDir(S1) then
    begin
  ...
  FHomeDir := ExtractDir(S1);
  if S <> '' then FConfigDir := ExtractDir(S)
  else begin FConfigDir := S1; SetRegConfigDir(ConfigDir);end;
...
{$ENDIF}
  ...
  begin
    ...
    begin
       ...
       GetMem(p, sz);
       d.Read(p^, sz);
       s.Addition.Write(p^, sz);
       s.Addition.Seek(-3, soFromEnd);
       b := s.Addition.ReadByte;
       if b = $FF then begin
          s.oVersion := s.Addition.ReadByte;
       end;
       s.Addition.Position := 0;
       FreeMem(p, sz);
       FreeObject(d);
    end;
...
function TElementOnly.Name: string; begin GlobalFail('%s', ['TElementOnly.Name']); result := '' end;
...
{$IFDEF USE_TAPI}
   if S = 'TAPI' then exit;
{$ENDIF}
   c := copy(s, 4, Length(s) - 4);
   Result := StrToIntDef(c, 0) - 1;
end;

function dLog: string;
begin
  Result := StrAsg(FullPath(IniFile.Log));
end;

//{$IFDEF RASDIAL}
//function Ras_Log: string;
//begin
//  Result := IniFile.ras_log;
//end;
//{$ENDIF}
...
begin
  Result := '';
  if IniFile = nil then exit;
  Result := StrAsg(FullPath(IniFile.Outbound));
end;
...
begin
  inherited Create;
  AddrList := TFidoAddrColl.Create('AddrList');
end;
...
function TPasswordColl.Password(const Addr: TFidoAddress): string;
var
  i, j: Integer;
  p: TPasswordRec;
begin
...
begin
  DoReadFile := False;
  if FirstRun then FHandle := _CreateFile(s, [cRead, cWrite, cExisting, cWriteThrough, cShareAllowWrite])
...
begin
  Result := False;
  s := '; Do not modify this file while Argus (or Argus brunch) is running, your changes may be overwritten'#13#10#13#10;
  for i := 0 to Count - 1 do
  begin
    r := At(i);
  ...
  if IniFile.CloseBWZFile then ZeroHandle(FHandle);
  Result := True;
end;
...
//  Result := False;
  ...
  CfgLeave;
  BWZColl := TBWZColl.Create('BWZColl');
  ...
  begin
    ...
    begin
      ...
      R := TBWZRec.Create;
      R.BWZNo := N;//номер BWZ
      R.FName := FNM;//реальное имя
      GetWrd(S, Z, ' ');
      R.FSize := Vl(Z);//реальный размер
      GetWrd(S, Z, ' ');
      R.FTime := Vl(Z);//время файла
      GetWrd(S, Z, ' ');
      R.TmpSize := Vl(Z);//временный размер
      GetWrd(S, Z, ' ');
      R.Prot := Vl(Z);//??
      GetWrd(S, Z, ' ');
      ParseAddress(Z, R.Addr);//Адрес

      if ((R.FSize = INVALID_VALUE) or
         (R.FTime = INVALID_VALUE) or
         (R.TmpSize = INVALID_VALUE) or
         (R.Prot = INVALID_VALUE)) or
      (R.TmpSize <> R.FSize) and (Nfo.Time + DWORD(IniFile.MaxBWZAge) < uGetSystemTime) then
      begin
        FreeObject(R);
        DeleteFile(FName);
      end else
  ...
  if IniFile.CloseBWZFile then ZeroHandle(BWZColl.FHandle);
  Result := True;
end;
...
begin
  Result := (UpperCase(r.FName) = uf) and ((r.FSize = FSize) or (FSize = 0)) and ((r.FTime = FTime) or (FTime = 0));
end;
...
begin
  CfgEnter;
  Result := StrAsg(MakeNormName(FullPath(IniFile.InTemp), Format(BWZFmt, [BWZNo])));
  CfgLeave;
end;
...
type
  TFreqScanner = class
    ...
    I: Integer;
    FreqData: TFreqData;
    procedure ScanDir(const Dir: string; ARecursive: Boolean);
...
procedure TReqRec.Add(const FName, APsw: string; const Info: TFileInfo);
var
  r: TReqFile;
begin
  if (APsw <> '') and (APsw <> Psw) then Exit;
  ...
  if Files = nil then Files := TColl.Create('Files');
  r := TReqFile.Create;
  r.FName := FName;
  r.Info := Info;
  Files.Add(r);
...
function _ValidFidoAddr(const SAddr:string; var NAdr:TFidoAddress):Boolean;
const
...
begin
  ...
  for i := 2 to sc.count - 2 do begin
     Addr := sc[i];
     if Addr[1] = '-' then continue;
     if not ValidInetAddr(Addr) then
     begin
       Addr := '"' + Addr + '"';
       if not ValidSymAddr(Addr) then Exit;
     end;
     if sc.Count > i + 1 then Pwd := sc[i + 1] else Pwd := '';
     InsertImportRec(Coll, a, '', Addr + ',CM,IBN', Pwd);
  end;
end;
...
procedure InsertIDovr(Coll: TColl; const AAddress: TFidoAddress; const Astr: string); forward;
...
procedure InsertIDovr(Coll: TColl; const AAddress: TFidoAddress; const Astr: string);
var
  aa: TFidoAddress;
...
begin
  case Typ of
    ...
    eiTimeDial: Result := FormatLng(rsTimeDial, [d.DwordData]);
    eiRetry: Result := FormatLng(rsTimeDial, [d.DwordData]);

    eiTrafShIn,
    eiTrafShOut: Result := FormatLng(rsRecsEcBPS, [d.DwordData]);
    ...
    eiRASEvent,
    eiSNDEvent : if d.BooleanData then Result := 'True'
                                  else Result := 'False';
...
begin
  case i of
    ...
    eiAccNoCram,
    eiLogEMSI,
    eiZMHEvent           : Result := eptVoid;
    eiLoginScript,
    eiModemScript        : Result := eptGrid;
    else begin
...
const
  ...
  'ZONE', 'NET', 'NODE', 'POINT', 'HZONE', 'DZONE', 'HNET', 'HNODE', 'HPOINT',
  'XZONE', 'XNET', 'XNODE', 'XPOINT', 'STATUS', 'TSTATUS', 'DOMAIN');
...
begin
  ...
  if inifile.D5Out then begin
     IntReplaced :=
       ReplaceCI('%STMAIL', '%DOMAIN.%XZONE%XNET%XNODE.%XPOINT%TSTATUS', s) or
       ReplaceCI('%LTMAIL', '%DOMAIN.%ZONE.%NET.%NODE.%POINT~/\x2E{0,1}/%TSTATUS', s);
  end else begin
     IntReplaced :=
...
} //commented by Denis Voituk. Needs to be rewritten to hold more than one filebox for a node 
...
begin
  o := GetRegHelpLng;
  RussianHelp := FileExists(GetHelpFile(HomeDir, LangNames[lnRussian]));
  EnglishHelp := FileExists(GetHelpFile(HomeDir, LangNames[lnEnglish]));
  GermanHelp := FileExists(GetHelpFile(HomeDir, LangNames[lnGerman]));
  DanishHelp := FileExists(GetHelpFile(HomeDir, LangNames[lnDanish]));
  DutchHelp := FileExists(GetHelpFile(HomeDir, LangNames[lnDutch]));
  SpanishHelp := FileExists(GetHelpFile(HomeDir, LangNames[lnSpanish]));
  if ((o = LangNames[lnRussian]) and RussianHelp) or
     ((o = LangNames[lnDutch]) and DutchHelp) or
     ((o = LangNames[lnEnglish]) and EnglishHelp) or
     ((o = LangNames[lnSpanish]) and SpanishHelp) or
     ((o = LangNames[lnDanish]) and DanishHelp) or
     ((o = LangNames[lnGerman]) and GermanHelp)
     then l := o else
  begin
    if EnglishHelp then l := LangNames[lnEnglish] else
    if RussianHelp then l := LangNames[lnRussian] else
    if DutchHelp then l := LangNames[lnDutch] else
    if SpanishHelp then l := LangNames[lnSpanish] else
    if DanishHelp then l := LangNames[lnDanish] else
    if GermanHelp then l := LangNames[lnGerman] else l := '';
    if o <> l then SetRegHelpLng(l);
  end;
  if l = LangNames[lnSpanish] then
  begin
    HelpLanguageId := HelpLanguageSpanish;
    Application.HelpFile := GetHelpFile(HomeDir, LangNames[lnSpanish]);
  end else
  if l = LangNames[lnDutch] then
  begin
    HelpLanguageId := HelpLanguageDutch;
    Application.HelpFile := GetHelpFile(HomeDir, LangNames[lnDutch]);
  end else
  if l = LangNames[lnGerman] then
  begin
    HelpLanguageId := HelpLanguageGerman;
    Application.HelpFile := GetHelpFile(HomeDir, LangNames[lnGerman]);
  end else
  if l = LangNames[lnDanish] then
  begin
    HelpLanguageId := HelpLanguageDanish;
    Application.HelpFile := GetHelpFile(HomeDir, LangNames[lnDanish]);
  end else
  if l = LangNames[lnRussian] then
  begin
    HelpLanguageId := HelpLanguageRussian;
    Application.HelpFile := GetHelpFile(HomeDir, LangNames[lnRussian]);
  end else
  if l = LangNames[lnEnglish] then
  begin
