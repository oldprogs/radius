...
uses Classes, Extctrls, Windows, SysUtils, SysConst;
...
{ TRxFolderMonitor changed to TArgusFolderMonitor to avoid conflicts }
  TArgusFolderMonitor = class(TComponent)
  private
...
constructor TArgusFolderMonitor.Create(AOwner: TComponent);
begin
...
destructor TArgusFolderMonitor.Destroy;
begin
...
procedure TArgusFolderMonitor.Loaded;
begin
  inherited Loaded;
  try
...
function TArgusFolderMonitor.GetActive: Boolean;
begin
  Result := FNotifyThread <> nil;
end;

procedure TArgusFolderMonitor.SetActive(Value: Boolean);
begin
...
procedure TArgusFolderMonitor.SetFilter(Value: TFileChangeFilters);
var
...
procedure TArgusFolderMonitor.SetMonitorSubtree(Value: Boolean);
begin
...
procedure TArgusFolderMonitor.SetFolderName(const Value: string);
begin
...
procedure TArgusFolderMonitor.FreeNotifyThread;
begin
...
procedure TArgusFolderMonitor.ResetNotifyThread(Activate: Boolean);
begin
  FreeNotifyThread;
...
function TArgusFolderMonitor.GetDelayTime: Cardinal;
begin
  if FDelayTimer <> nil then
...
procedure TArgusFolderMonitor.SetDelayTime(Value: Cardinal);
begin
...
procedure TArgusFolderMonitor.ThreadNotification(Sender: TObject);
begin
...
procedure TArgusFolderMonitor.Timer(Sender: TObject);
begin
  FDelayTimer.Free;
  FDelayTimer := nil;
  Changed;
...
procedure TArgusFolderMonitor.Changed;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;
...
begin
{ Components }
  RegisterComponents('Argus', [TArgusFolderMonitor]);
end;
